...
def show_countdown(countdown_seconds, audio_file_path):
    """
    显示一个全屏窗口来进行倒计时，并播放音频。
    ...
    try:
        ...
        def countdown(count):
            ...
            if count >= 0:
                ...
                try:
                    play_audio_from_last_x_seconds(audio_file_path, countdown_seconds)
                except Exception as e:
                    app.write_log(f"Error playing audio: {e}")
                root.destroy()  # 倒计时到0后关闭窗口
...
def play_audio_from_last_x_seconds(audio_file_path, countdown_seconds):
    """
    使用 simpleaudio 播放音频文件，从倒数 countdown_seconds 秒开始播放。
    ...
    try:
        ...
        audio_to_play.export("temp_audio.wav", format="wav")
        
        wave_obj = sa.WaveObject.from_wave_file("temp_audio.wav")
        play_obj = wave_obj.play()
        play_obj.wait_done()
    except Exception as e:
        app.write_log(f"Error processing audio file: {e}")
...
def play_audio_and_show_countdown(audio_file_path, countdown_seconds):
    ...
    try:
        ...
        show_countdown(countdown_seconds, audio_file_path)
    except Exception as e:
